...
Solver::Solver(Instance* instance) : instance(instance) {
	...
	for (int i = 0; i < E; i++)
		for (int j = 0; j < E; j++)
			conflict_matrix[i][j] = instance->conflict_matrix[i][j];
...
void Solver::Simple3DModel() {
	...
	for (int j = 0; j < E; j++) {
		...
		model.add(expr6 <= numberOfStudentInExam[j] / capacity[0] + 1).setName(("C9'_j_" + to_string(j)).c_str());
	}
		...
		{
			{
				...
				for (int t = 0; t < T; t++) {
					model.add(wi_s[i] <= t + T * (1 - w_it[i][t])).setName(("C11s_i_t" + to_string(i)
						+ "," + to_string(t)).c_str());
					model.add(wi_e[i] >= t * w_it[i][t]).setName(("C11e_i_t" + to_string(i)
						+ "," + to_string(t)).c_str());
				}
			}
			model.add((int)examOfStudent[i].size() - 1 <= wi_e[i] - wi_s[i]).setName(("C11d_i_" + to_string(i)).c_str());
	...
	for (int j1 = 0; j1 < E; j1++) {
		for (int j2 = j1 + 1; j2 < E; j2++)
			if (conflict_matrix[j1][j2])
				for (int t = 0; t < T; t++)
					model.add(x_jt[j1][t] + x_jt[j2][t] <= 1);
	}

	//room compresing
	for (int r = 0; r + 1 < R; r++)
		for (int t = 0; t < T; t++)
			model.add(x_rt[r][t] >= x_rt[r + 1][t]);
	...
	for (int j = 0; j < E; j++) {
		for (int r = 0; r < R; r++) {
			for (int t = 0; t < T; t++) {
				obj3 += 1e5 * x_jrt[j][r][t];
			}
		}
	}

	IloExpr obj5(env); // penalty cho so phong phai su dung
	{
		for (int r = 0; r < R; r++) {
			for (int t = 0; t < T; t++) {
				obj5 += 1e5 * x_rt[r][t];
	...
	for (int j = 0; j < E; j++)
		//for (int t = 0; t < T; t++)
		model.add(SLOT >= zi_e[j]);


	model.add(IloMinimize(env, obj1 + obj2 + obj5 + obj3 + SLOT * 1e9));
	...
	cout << "\n\nResult starts here:\n\n";
	cout << "Phòng thi, thứ tự thi và sinh viên thi ứng với từng môn:";
	outFile << "Phòng thi, thứ tự thi và sinh viên thi ứng với từng môn:";

	for (int j = 0; j < E; j++) {
		cout << "\n" << j+1 << " " << decoding_exam[j] << ": " << exam_and_infor[decoding_exam[j]];
		outFile << "\n" << j+1 << " " << decoding_exam[j] << ": " << exam_and_infor[decoding_exam[j]];
		for (int r = 0; r < R; r++) {
			for (int t = 0; t < T; t++) {
				if (cplex.getValue(x_jrt[j][r][t]) == 1) {
					...
					for (int i = 0; i < S; i++) {
						if (examOfStudent[i].count(j) == 1 && cplex.getValue(w_ijt[i][j][t]) == 1 && cplex.getValue(w_irt[i][r][t]) == 1) {
							cnt++;
							cout << "\n\t\t\t" << cnt << " " << decoding_student[i] << " " << student_and_infor[decoding_student[i]];
							outFile << "\n\t\t\t" << cnt << " " << decoding_student[i] << " " << student_and_infor[decoding_student[i]];
						}
					}
				}
	...
	cout << endl;
	*/
	cout << "\n\nMôn thi, thời gian thi, sinh viên thi ứng với từng phòng: \n";
	outFile << "\n\nMôn thi, thời gian thi, sinh viên thi ứng với từng phòng: \n";
	for (int r = 0; r < R; r++) {
		outFile << "Room: " << r;
		cout << "Room: " << r;
		for (int t = 0; t < T; t++) {
			for (int j = 0; j < E; j++) {
				if (cplex.getValue(x_jrt[j][r][t])) {
					outFile << "\nTime slot: " << t << " | " << decoding_exam[j] << ": " << exam_and_infor[decoding_exam[j]];
					cout << "\nTime slot: " << t << " | " << decoding_exam[j] << ": " << exam_and_infor[decoding_exam[j]];
					int cnt = 0;
					for (int i = 0; i < S; i++) {
						if (examOfStudent[i].count(j) == 1 && cplex.getValue(w_ijt[i][j][t]) == 1 && cplex.getValue(w_irt[i][r][t]) == 1) {
